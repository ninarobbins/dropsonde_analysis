...
def quicklooks_l2_profiles(flight_id, ds_list, filepath_list, loc_list, save_path):
    ...
    fig, axs = plt.subplots(1, 5, figsize=(18, 8), sharey=True)  # 1 row, 5 columns, adjusted figure size
    fs = 14
    ...
    for i, var in enumerate(variables):
        for j, sonde in enumerate(ds_list):
            ...
            color = cmap(j)
            axs[i].scatter(sonde[var], sonde.gpsalt, c=color, s=10, label=time_only+f' ({loc_list[j]})')
...
def get_mean_launch_time(ds_flight=None, time_format="%Y%m%d %H:%M:%S"):
    ...
    mean_time = convert_time_to_str(ds_flight.interpolated_time[0].mean().values, time_format)
